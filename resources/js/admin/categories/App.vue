      ...
      <div class="p-8 max-w-6xl mx-auto">
        ...
        <div class="mb-8 p-6 bg-slate-800/50 backdrop-blur-xl border border-white/5 rounded-2xl shadow-xl">
          <h2 class="text-lg font-bold text-white mb-4 flex items-center">
            <span class="mr-2">{{ form.id ? 'Edit Category' : 'Add New Category' }}</span>
            <div class="h-px bg-white/10 flex-1 ml-4"></div>
          </h2>
          
          <form @submit.prevent="save" class="flex gap-4 items-end">
            <div class="flex-1 space-y-2">
              <label class="text-sm font-medium text-slate-400 ml-1">Category Name</label>
            ...
            <button class="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-semibold py-3 px-6 rounded-xl shadow-lg shadow-indigo-500/20 transform transition-all duration-200 hover:scale-105 active:scale-95 flex items-center">
        ...
        <div class="bg-slate-800/50 backdrop-blur-xl border border-white/5 rounded-2xl shadow-xl overflow-hidden">
          <div class="p-6 border-b border-white/5 flex justify-between items-center">
            ...
            <span class="text-sm text-slate-400 bg-white/5 px-3 py-1 rounded-lg border border-white/5">{{ categories.length }} entries</span>
          </div>
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-white/5">
                  ...
                  <th class="p-4 w-full">Name</th>
                  <th class="p-4 pr-6 text-right">Actions</th>
                </tr>
              </thead>
              <tbody class="text-sm divide-y divide-white/5">
                <tr v-for="(c,i) in categories" :key="c.id" class="hover:bg-white/5 transition-colors duration-150 group">
...
const load = async () => {
    try {
        const res = await fetch('/admin/categories/data');
        if (res.ok) {
...
const executeDelete = async () => {
    ...
    try {
        await fetch(`/admin/categories/${itemToDelete.value}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            }
